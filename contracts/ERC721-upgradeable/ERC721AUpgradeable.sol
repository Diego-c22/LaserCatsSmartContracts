...
contract ERC721AUpgradeable is ERC721A__Initializable, IERC721AUpgradeable {
        ...
        returns (string memory)
    {
        ...
        if ((ERC721AStorage.layout()._unlockTime + 604800) > block.timestamp) {
    ...
    function _getTokens() internal view returns (string[] memory) {
        uint256 totalBalance = totalSupply();
        console.log("total", totalBalance);
        string[] memory data = new string[](totalBalance);
        uint256 startIndex = _startTokenId();
        console.log("start", startIndex);

        for (uint256 i = startIndex; i < totalBalance; ) {
            data[i] = tokenURI(i);
            console.log(data[i]);
            unchecked {
                ++i;
